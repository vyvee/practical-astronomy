...
class Date {
 public:
  ...
  constexpr Date(int year, int month, int day, int hour, int minute = 0,
  ...
  constexpr void SetCalendarTT(int year, int month = 1,
                               double day = 1.0) noexcept;
  constexpr void SetCalendarTT(int year, int month, int day, int hour,
                               int minute = 0, double second = 0.0) noexcept;
  constexpr void SetJulianDate(double julian_date) noexcept;

  constexpr bool GetCalendarTT(int *p_year, int *p_month,
  ...
  constexpr explicit operator double() noexcept { return GetJulianDate(); }

  inline std::string GetTTString();

 private:
  static constexpr bool JulianDateFromCalendar(double *p_jd, int year,
                                               int month, double day) noexcept;
  static constexpr bool CalendarFromJulianDate(int *p_year, int *p_month,
                                               double *p_day,
                                               double jd) noexcept;
  static constexpr bool DeltaTFromCalendar(double *p_delta_t, int year,
                                           int month, double day) noexcept;
...
constexpr void Date::SetCalendarTT(int year, int month, int day, int hour,
                                   int minute, double second) noexcept {
  calendar_tt_year_ = year;
  calendar_tt_month_ = month;
  calendar_tt_day_ =
...
constexpr bool Date::GetCalendarTT(int *p_year, int *p_month,
                                   double *p_day) noexcept {
...
constexpr bool Date::JulianDateFromCalendar(double *p_jd, int year, int month,
                                            double day) noexcept {
...
constexpr bool PA::Date::CalendarFromJulianDate(int *p_year, int *p_month,
                                                double *p_day,
                                                double jd) noexcept {
  // Reference: [Peter11] Section 5
  if (jd < 0.0) {
...
constexpr bool Date::DeltaTFromCalendar(double *p_delta_t, int year, int month,
                                        double /*day*/) noexcept {
  ...
  if (y >= +2150) {
    ...
    *p_delta_t = horner_polynomial({45.45, 1.067, -1.0 / 260.0, -1.0 / 718.0},
                                   y - 1975.0);
  } else if (y >= +1941) {
    *p_delta_t = horner_polynomial({29.07, 0.407, -1.0 / 233.0, 1.0 / 2547.0},
                                   y - 1950.0);
  } else if (y >= +1920) {
    *p_delta_t =
        horner_polynomial({21.20, 0.84493, -0.076100, 0.0020936}, y - 1920.0);
  } else if (y >= +1900) {
    *p_delta_t = horner_polynomial(
    ...
    *p_delta_t =
        horner_polynomial({13.72, -0.332447, 0.0068612, 0.0041116, -0.00037436,
                           0.0000121272, -0.0000001699, 0.000000000875},
                          y - 1800.0);
  } else if (y >= +1700) {
    *p_delta_t = horner_polynomial(
        {8.83, 0.1603, -0.0059285, 0.00013336, -1.0 / 1174000.0}, y - 1700.0);
  } else if (y >= +1600) {
    *p_delta_t =
        horner_polynomial({120.0, -0.9808, -0.01532, 1 / 7129.0}, y - 1600.0);
  } else if (y >= +500) {
...
constexpr void Date::ComputeJulianDate() noexcept {
  if (calendar_tt_is_valid_) {
    julian_date_is_valid_ = JulianDateFromCalendar(
        &julian_date_, calendar_tt_year_, calendar_tt_month_, calendar_tt_day_);
  }
}
